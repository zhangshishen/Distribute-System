...
func doMap(
	...
	kv := mapF(inFile,string(dat))
	//print(string(dat))
	//f,_ = os.Create("temptext.txt")
	
	jsonMap := make([]*json.Encoder,nReduce)

	for i:=0;i<nReduce;i++ {
		file,_ := os.Create(reduceName(jobName,mapTaskNumber,i))
		jsonMap[i] = json.NewEncoder(file)
	}

	for _,KV := range kv {
		//ioutil.WriteFile("texp.txt",KV.Key+" "+KV.Value,0644)
		jsonMap[ihash(KV.Key)%nReduce].Encode(&KV)
	}
